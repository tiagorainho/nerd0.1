        ...
        <div class="row mh-100vh">
            <div class="col-10 col-sm-8 col-md-6 col-lg-6 offset-1 offset-sm-2 offset-md-3 offset-lg-0 align-self-center d-lg-flex align-items-lg-center align-self-lg-stretch bg-white p-5 rounded rounded-lg-0 my-5 my-lg-0" id="login-block">
                <div class="m-auto w-lg-75 w-xl-50">
                    <h2 class="text-info font-weight-light mb-5"><i class="fa fa-diamond"></i>&nbsp;Apagar Material</h2>
                    
                    
                    <form method="post" action="<?php echo $_SERVER['PHP_SELF'];?>">
                       
                       
                        <div class="form-group"><label class="text-secondary">Nome do Objeto</label>
                ...
                if($_SERVER["REQUEST_METHOD"]=="POST"){
                    ...
                    $query="SELECT * FROM inventario WHERE nome='$nome' and active='0'";
                    $run_query= mysqli_query($connection,$query);
                    $result=mysqli_num_rows($run_query);
                    ...
                    if($result==1){
                        $query="SELECT * FROM apagar WHERE inventario='$password'";
                        $run_query= mysqli_query($connection,$query);
                        $result=mysqli_num_rows($run_query);
                        
                        if($result==1){
                            
                            $query_apagar="UPDATE inventario SET active = '1' WHERE nome = '$nome'";
                            $run_query = mysqli_query($connection,$query_apagar);
                            
                            $query_a= "INSERT INTO inventario_apagado (id,nome,user,date_time) VALUES ('NULL','$nome','$user','$today')";
                            $run_query_apagar = mysqli_query($connection,$query_a);
                            
                            if($run_query){
                                 echo "<p style= color:green>Objeto &quot$nome&quot apagado</p>";
                            }
                            else{
                                 echo "<p style= color:red>Objeto &quot$nome&quot não foi apagado</p>";
                            }
                        ...
                        echo "<p style=color:orange>Objeto &quot$nome&quot não encontrado</p>";
                    }
                    else{
                        ?>
