    ...
    <main class="page users-page">
        <section class="portfolio-block projects-cards">
            <div class="container" align="center">
                <div class="heading">
                    <h5 style="color:grey">Validar código de autentificação</h5>
                </div>        
                
                <?php
               
                    if($_SERVER["REQUEST_METHOD"]=="POST"){
                    $raw_confirmacao=trim($_POST['raw_confirmacao']);
                        
                    $confirmacao=filter_var($raw_confirmacao,FILTER_SANITIZE_STRING);
                        ...
                        if(strlen($codigo_validacao)!=0){
                            if($codigo_validacao==$confirmacao){
                                $query_update="UPDATE data SET tentativa = '$confirmacao' WHERE nome = '$nome' and password='$password'";
                                ...
                                header("Location:mudar_password.php");
                                    ...
                                    ?>
                                        <p style="color:red">Código de validação errado!</p>
                                    <?php
                                }
                        }
                        else{
...
/*
                ...
                <form method="post" action="<?php echo $_SERVER['PHP_SELF']?>">
                        <div class="form-group"><label class="text-secondary">Código</label>
                        <input name=raw_confirmacao class="form-control" type="text" required="" inputmode="text"></div>
                        <button class="btn btn-info mt-2" type="submit">Validar</button>
                        <div align="left">
                            <p class="mt-3 mb-0"><a href="criar_codigo_autentificacao.php" class="text-info small">Voltar a enviar email</a></p>
                        </div>
                </form>
